...
# =====================================================================================================================
use constant PYTHON_X => '~/software/Python/bin/python3.7 -W ignore';
...
our @EXPORT_OK = qw(run_jobs PYTHON_X);
...
sub run_jobs {
    ...
    if (my $pid = fork()) {
        ...
        for my $job (@$jobs) {
            while (1) {
                if (scalar @{ $ssh_runner->MY() } < MAX_JOBS + 1) {
                    ...
                    $ssh_runner->exec('echo "' . $job->{config} . '" > ' . 'job_' . $job->{id} . '_config.py');
        ...
        my $ssh_waiter = SSH->new(
            host    => $ssh_runner->{host},
            user    => $ssh_runner->{user},
            pass    => $ssh_runner->{pass},
            rootdir => $ssh_runner->{rootdir}
        );

        $ssh_waiter->connect();

        $ssh_waiter->cd($ssh_waiter->{rootdir});
                ...
                if ($job->{task_id} != -1) {
                        ...
                        && $job->{status} ne 'fail')
                    {
                        if (defined $job_info->{status}) {
                            ...
                            # -------------------------------------------------------------------------
                            # download output
                            $ssh_waiter->receive_files($job->{output}, $job_folder . '/' . $job->{task_id} . '.out');
                            ...
                            # -------------------------------------------------------------------------
                            # download error
                            my ($o, $e) = $ssh_waiter->cmd('wc -c ' . $job->{path} . '/' . $job->{error});

                            $o->[0] =~ /^(?<size>\d+)\s+/;

                            if ($+{size} == 0) {
                                if ($to_download) {
                                    $ssh_waiter->receive_files($job->{sc_out_dir},
                                ...
                                $ssh_waiter->receive_files($job->{error}, $job_folder . '/' . $job->{task_id} . '.err');
                                $job->{status} = 'fail';
                            }

                            $ssh_waiter->exec('rm ' . $job->{error});
                            $ssh_waiter->exec('rm ' . 'job_' . $job->{id} . '_config.py');
